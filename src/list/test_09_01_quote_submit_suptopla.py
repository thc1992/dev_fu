...
class Test_quote_submit_suptopla:
    @allure.description("查看详情")
    @allure.suite('报价')
    @allure.title("报价")  # 测试用例的标题
    @allure.testcase('/fsm-platform/fee/actual/workOrder/submit')
    @pytest.mark.one
    def test_quote_submit_suptopla(self):
        quoteId_data = Read_sql_Data.Conte(quoteId_suptopla)[0][0]
        quote_submit = Read_sql_Data.Conte(result_quoteId_suptopla % str(quoteId_data))
