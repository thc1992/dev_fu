...
class Test_quote_submit_platobrand:
    @allure.description("查看详情")
    @allure.suite('报价')
    @allure.title("报价")  # 测试用例的标题
    @allure.testcase('/fsm-platform/fee/actual/workOrder/submit')
    @pytest.mark.one
    def test_quote_submit_platobrand(self):
        quoteId_data = Read_sql_Data.Conte(quoteId_platobrand)[0][0]
        quote_submit = Read_sql_Data.Conte(result_quoteId_platobrand % str(quoteId_data))
